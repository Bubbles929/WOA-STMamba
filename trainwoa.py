    ...
    for epoch in range(1, cfg.n_epochs + 1):
        ...
        for start in range(len(x_train) - cfg.batch_size + 1):
            ...
            if start % int((len(x_train) - cfg.batch_size) / 5) == 0:
                print('epoch: {}  progress: {:.0f}%  loss: {:.3f}  mae: {:.3f}'.format(epoch, progress * 100, loss, mae_train_batch))
            cnt += 1
...
def main(X):
    ...
    #7
    cfg.out_in_channels = round(X[0])
    cfg.lr = round(X[1],6)
    cfg.d_state = round(X[2])
    cfg.d_conv = round(X[3])
    cfg.expand = round(X[4])
    cfg.d_inner = round(X[5])
    cfg.dt_rank = round(X[6])

############mamba
#     cfg.d_state = round(X[0])
#     cfg.d_conv = round(X[1])
#     cfg.expand = round(X[2])
#     cfg.d_inner = round(X[3])
#     cfg.dt_rank = round(X[4])
# #############
    # Load data
    print('\nLoading data...\n')
    ...
    if cfg.model_name == 'RNN':
        ...
        net = models.SimpleGRU(input_size=cfg.input_size, hidden_size=cfg.hidden_size, output_size=cfg.output_size, num_layers=cfg.num_layers, in_channels=cfg.in_channels,
        ...
        net = models.oMamba(input_size=cfg.input_size, in_channels=cfg.in_channels, out_in_channels=cfg.out_in_channels,d_state=cfg.d_state, d_conv=cfg.d_conv, expand=cfg.expand,
    ...
    # Training
    print('\nStart training...\n')
    print(cfg.out_in_channels, cfg.lr, cfg.d_state, cfg.d_conv, cfg.expand, cfg.d_inner, cfg.dt_rank)

    # rmse_valid_list = train(net, x_train, y_train, x_valid, y_valid, x_test, y_test)
    # min_rmse_valid = np.min(rmse_valid_list)

    mae_valid_list = train(net, x_train, y_train, x_valid, y_valid, x_test, y_test)
    min_mae_valid = np.min(mae_valid_list)
    return min_mae_valid
